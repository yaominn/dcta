...
@pytest.fixture
def server_url():
    """Re-seed the mock ledger (known balance 842050, no passkeys), start
    uvicorn on a free port with WEBAUTHN_EXPECTED_ORIGIN matching that port,
    wait for health, then tear down + re-seed so the repo DB is left clean."""
