...
executor does no arithmetic — it only debits the resolved dollars.
"""
...
class MockExecutor:
            ...
            for leg in plan.plan:
                ...
                try:
                    if leg.type == "TRANSFER":
                        self._debit(conn, leg.source_account, float(leg.amount))
                    ...
                    results.append({"id": leg.id, "type": leg.type,
                                    "status": "EXECUTED", "amount": float(leg.amount)})
    ...
    def _debit(self, conn, account_id: str, amount: float) -> None:
        ...
        bal = row["balance"]
        if bal < amount:
            raise RuntimeError(f"insufficient funds in {account_id}: {bal:.2f} < {amount:.2f}")
        conn.execute("UPDATE accounts SET balance=? WHERE id=?", (round(bal - amount, 2), account_id))
